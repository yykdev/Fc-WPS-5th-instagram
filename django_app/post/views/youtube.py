...
def youtube_search(request):
    ...
    if q:
        ...
        for item in items['items']:
            ...
            exists = YouTube_Video.objects.filter(youtube_id=youtube_id).exists()
            if not exists:
                video = YouTube_Video.objects.create(youtube_id=youtube_id, channelId=channelId, title=title,
                                                     thumbnails=thumbnails)
                videos.append(video)
            else:
                video = YouTube_Video.objects.get(youtube_id=youtube_id)
                videos.append(video)
        context = {
            'videos': videos,
        }
...
def post_video(request, youtube_id):
